...
import java.sql.*;
class SendReport extends JFrame{
...
jb1=new JButton("show");
    	...
    	try{
    	    ...
    	    rs=st.executeQuery("Select date,debit,transferaccnum from transaction where accountnum = '"+value+"' and    type = '::Balance Trans' and debit !='0'");
    ...
    try{
    	dataValues=new String[rows][3];
				 ...
				 columnNames=new String[]{"Date","Send Ammount","To(Account Number)"};
				 rs=st.executeQuery("Select date,debit,transferaccnum  from transaction where accountnum = '"+value+"' and   type = '::Balance Trans' and debit !='0'");
