...
import java.sql.*;
class ReceiveReport extends JFrame{
...
JButton jb1;
public  ReceiveReport(beans acc){
   setTitle("Receive Report");
    setSize(500,500);
setDefaultCloseOperation (DISPOSE_ON_CLOSE);
title =new JLabel("Show Received History");
...
jb1=new JButton("show");
    	...
    	try{
    	    ...
    	    rs=st.executeQuery("Select date,credit,transferaccnum from transaction where accountnum = '"+value+"' and    type = '::Balance Trans' and credit !='0'");
    ...
    try{
    	dataValues=new String[rows][3];
				 ...
				 columnNames=new String[]{"Date","Received Ammount","From(Account Number)"};
				 rs=st.executeQuery("Select date,credit,transferaccnum  from transaction where accountnum = '"+value+"' and   type = '::Balance Trans' and credit !='0'");
